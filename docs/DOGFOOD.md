...
This is an internal dogfood report for the v0.4 Reliability Contract, v0.4.1 public-proof follow-up, v0.4.2 false-positive failure-detection hardening, and v0.4.3 CLI discoverability hardening. It is not an independent benchmark and it does not claim to control OpenAI server-side compaction. It measures whether Sentinel preserves the local continuity contract around Codex Desktop: objective, next action, evidence, loop warnings, redaction, and project resolution.
