...
This is an internal dogfood report for the v0.4 Reliability Contract, v0.4.1 public-proof follow-up, and v0.4.2 false-positive failure-detection hardening. It is not an independent benchmark and it does not claim to control OpenAI server-side compaction. It measures whether Sentinel preserves the local continuity contract around Codex Desktop: objective, next action, evidence, loop warnings, redaction, and project resolution.
...
| React/Electron app | `X Growth Lab`, `package.json`, installed app bundle resources | `cs packet --cwd` resolved the project |
| iOS/Capacitor app | `emerald-singularity`, `package.json`, `capacitor.config.ts` | `cs packet --cwd` resolved the project |
| Large Unity repo | `OFFDuty-codex-phase7-tps-rig`, `ProjectSettings` | `cs packet --cwd` resolved the project |
...
The next credibility step is to collect more anonymized user-submitted dogfood reports using `cs export --project --cwd "$PWD"` output with secrets scrubbed.
