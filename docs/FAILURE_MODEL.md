...
- Repeated test failures with no new hypothesis.
- Objective changes midway through a task.
- Tool result says failed, but the assistant proceeds as if it passed.
- Permission approval requests repeat without preserving context.
...
- Project switching leaks state between workspaces.
- Stop auto-continue creates its own continuation loop.
...
- Redaction: the pattern set is broad but cannot guarantee every possible custom secret format.
...
Replay fixtures in `tests/fixtures/scenarios/` cover the most important failure modes. Add a fixture before adding a new continuity feature whenever possible.
